...
import { ShoppingItem, IngredientCategory, CATEGORY_LABELS } from '@/types';
...
export const ShoppingListView: React.FC<ShoppingListViewProps> = ({
  ...
  const groupedItems = useMemo(() => {
    ...
    // Inicializar categorías ordenadas
  ...
  const handleShareWhatsApp = () => {
    const text = formatShoppingListForShare(items, weekRange);
    ...
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 no-print">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Generada automáticamente según el menú de la semana <span className="font-semibold text-slate-700">({weekRange})</span>.
          </p>
        </div>
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs no-print">
        <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
          ...
          <span>Añadir producto extra a la lista (despensa, limpieza, caprichos...)</span>
        </h3>

        <form onSubmit={handleAddCustomSubmit} className="flex flex-col sm:flex-row items-center gap-2">
          <input
            type="text"
            placeholder="Nombre del producto (ej. Leche entera, Papel cocina)..."
            value={customName}
      ...
      {totalCount === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-200">
          ...
          <h3 className="font-bold text-slate-800 text-base mt-3">Tu lista de compra está vacía</h3>
          <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-md mx-auto">
            Planifica platos en el <strong>Menú Semanal</strong> y pulsa &ldquo;Sincronizar menú&rdquo;, o añade artículos manualmente arriba.
          </p>
        </div>
      ) : (
